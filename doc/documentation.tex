...
The net models a barrier with $N = 3$ parties (the number used during JPF
verification: two \texttt{WorkerThread}s and the \texttt{UpdateThread}). Each
party is represented by a dedicated input place $P_{i}$ carrying an initial
token, and a dedicated output place $P_{i}'$. A single transition
\textit{barrier fires} consumes one token from each input place and produces
one token in each output place.

%\begin{figure}[h]
%\centering
%\includegraphics[width=0.75\textwidth]{petri_cyclic_barrier}
%\caption{Petri Net modelling the \texttt{CyclicBarrier} with $N = 3$ parties.
%         The transition \textit{barrier fires} is enabled only when all three
%         input places hold a token simultaneously.}
...
if and only if all $N$ input places simultaneously hold a token. Since every
party eventually deposits its token (no alternative path exists in the net),
the transition is guaranteed to fire. No subset of parties can block the others
indefinitely.

\textbf{Liveness.} After \textit{barrier fires}, every output place $P_{i}'$
receives a token, allowing all parties to proceed. The net is live: every
transition that is ever enabled will eventually fire in any fair execution.

\textbf{Cyclic reset.} The output places $P_{i}'$ are connected back to the
input places $P_{i}$ of the next barrier cycle (not shown in the figure for
clarity), restoring the initial marking and making the barrier ready for the
next tick without reallocation.

\textbf{Boundedness.} The net is 1-safe: at most one token can reside in any
place at any time. This follows from the fact that each party deposits exactly
one token per cycle, and the transition consumes all tokens atomically.
