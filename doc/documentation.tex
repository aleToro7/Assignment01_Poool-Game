...
Da valutare qualcosa da aggiungere se diverse dalla parte task based, altrimenti unificare
...
The grid must be rebuilt every tick because ball positions change in phase~2. Rather than reconstructing it in the sequential section (which would waste the time between the two barriers), the rebuild is assigned exclusively to \textbf{Worker-0} (\texttt{isGridBuilder == true}). All other workers reach \texttt{barrierGrid.await()} immediately and block. Worker-0 calls \texttt{grid.build(allBalls)}, which clears the map and re-inserts every ball into its cell in $O(n)$, then also calls \texttt{barrierGrid.await()}.
 
When \texttt{barrierGrid} fires, the Java Memory Model guarantees a \emph{happens-before} relationship between Worker-0's writes to the grid and all subsequent reads by the other workers in phase~2: no additional synchronization is
needed to read the grid safely.
 
The three \texttt{CyclicBarrier} instances now partition each tick into four synchronization points:
 
\[
\text{barrierPositions} \;\to\; \text{barrierGrid} \;\to\; \text{barrierCollisions}
\]
 
Each barrier is constructed with $\mathit{parties} = N + 1$ (one per worker plus the \texttt{UpdateThread}), so the \texttt{UpdateThread} participates in all three barriers and proceeds to the sequential phases only after every worker has completed phase~3.
...
\texttt{allBalls} snapshot. This allows $O(1)$ index lookup during phase~3,
...
The diagram covers five phases. In \textbf{Phase 0}, the \texttt{UpdateThread} sequentially calls \texttt{board.updatePlayers(dt)} before any parallel work begins. In \textbf{Phase 1}, the \texttt{UpdateThread} dispatches \texttt{startTick()} to all workers and immediately blocks on \texttt{barrierPositions.await()}, while \texttt{WorkerThread-0} and \texttt{WorkerThread-1..N} update the positions of their respective ball partitions in parallel. The first red dashed line marks the firing of \texttt{barrierPositions}, after which all threads are guaranteed to have completed position updates. In \textbf{Phase G}, only \texttt{WorkerThread-0} performs work — rebuilding the \texttt{SpatialGrid} from the updated positions — while all other threads block immediately on \texttt{barrierGrid.await()}. The asymmetry in activation box lengths communicates this visually: \texttt{WorkerThread-0} has a tall box while the others have near-zero boxes. The second red dashed line marks the firing of \texttt{barrierGrid}, establishing the happens-before relationship that allows all workers to safely read the grid in the next phase. In \textbf{Phase 2}, all workers resolve ball-ball collisions in parallel using the spatial grid, acquiring per-ball locks in canonical index order to prevent deadlock, while the \texttt{UpdateThread} again blocks immediately. The third red dashed line marks the firing of \texttt{barrierCollisions}. Finally, in \textbf{Phases 3 and 4}, the \texttt{UpdateThread} exclusively calls \texttt{board.resolvePlayerCollisions()} and \texttt{board.checkHolesAndGameOver()} sequentially, then signals the \texttt{BotThread} via \texttt{signal()} before updating the \texttt{ViewModel} and triggering a render.
